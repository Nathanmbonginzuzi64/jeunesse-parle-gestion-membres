    ...
    <ul className="space-y-3">
      {data.map((province) => {
        ...
        const structureCount =
          (province.structures?.length ?? 0) +
          (province.cities?.reduce((acc, city) => acc + (city.structures?.length ?? 0), 0) ?? 0);
                    ...
                    <div key={city.id} className="mt-2 ml-3 rounded-xl border border-slate-100 bg-slate-50/50 p-2">
                      ...
                      >
                        ...
                        <span className="ml-auto text-xs font-normal text-slate-500">
                          {(city.structures ?? []).length} structure(s)
                        </span>
                      </button>
                      {cityOpen && (
                        <ul className="ml-4 space-y-1 border-l-2 border-brand-200 pl-3 pt-1">
                          {(city.communes ?? []).map((commune) => (
                            <li key={commune.id} className="py-0.5 text-xs text-slate-500">
