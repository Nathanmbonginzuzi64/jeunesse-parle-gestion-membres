...
import { Activity, MonitorSmartphone, ScrollText, Shield, Smartphone } from "lucide-react";
    ...
    <div className="relative overflow-hidden rounded-card border border-slate-700/40 bg-gradient-to-br from-slate-900 via-slate-800 to-brand-950 px-5 py-4 text-white shadow-[var(--shadow-elevated)] sm:px-6 sm:py-5">
      ...
      <div className="relative flex flex-wrap items-start justify-between gap-4">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 max-w-xl text-sm text-brand-100/90">
              Toutes les actions des utilisateurs — portail web et application mobile — en temps réel.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 text-xs">
          {live ? (
            <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/25 px-3 py-1.5 font-medium ring-1 ring-inset ring-emerald-300/40">
          ...
          {today !== undefined && (
            <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/20 px-3 py-1.5 font-medium ring-1 ring-inset ring-emerald-300/30">
              ...
              {formatNumber(today)} aujourd&apos;hui
            </span>
          )}
          {web !== undefined && (
            <span className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-1.5 font-medium ring-1 ring-inset ring-white/15">
              <MonitorSmartphone className="h-3.5 w-3.5" />
              Web {formatNumber(web)}
            </span>
          )}
          {mobile !== undefined && (
            <span className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-1.5 font-medium ring-1 ring-inset ring-white/15">
              <Smartphone className="h-3.5 w-3.5" />
              Mobile {formatNumber(mobile)}
            </span>
          )}
        </div>
      </div>
